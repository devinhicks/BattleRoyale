    ...
    void Update()
    {
        if (shrinking)
        {
            transform.localScale = Vector3.MoveTowards(transform.localScale,
                Vector3.one * targetDiameter, (shrinkAmount / shrinkDuration)
                * Time.deltaTime);
        ...
        {
            // can we shrink again?
            if (Time.time - lastShrinkEndTime >= shrinkWaitTime &&
                transform.localScale.x > minShrinkAmount)
                Shrink();
        }
    ...
    void CheckPlayers()
    {
        if(Time.time - lastPlayerCheckTime > 1.0f)
        {
            ...
            foreach(PlayerController player in GameManager.instance.players)
            {
                ...
                if (Vector3.Distance(Vector3.zero, player.transform.position) >=
